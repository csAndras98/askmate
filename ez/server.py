...
"""
@app.route('/question/<question_id>/<answer_id>/new-message', methods=['GET', 'POST'])
def route_new_message(question_id=None, answer_id=None):
    if request.method == 'POST':
        data_manager.save_massage(answer_id, request.form['note'])
        return redirect(f'/question/{question_id}')
    return render_template('new-message.html', question_id=question_id, answer_id=answer_id)
"""
